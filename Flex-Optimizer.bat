...
echo     [4] Optimize DNS Settings (Cloudflare 1.1.1.1)
echo     [5] Force Restart Network Adapter
echo     [6] Backup Entire Registry to Downloads
echo --------------------------------------------------
...
:BACKUP
echo.
echo Backing up full registry to your Downloads folder...
set DOWNLOADS=%USERPROFILE%\Downloads
set BACKUP_FILE=%DOWNLOADS%\flex_registry_backup.reg
...
if exist "%BACKUP_FILE%" (
    ...
    echo Backup failed. Please make sure you have permission.
)
goto ASK
...
:DNS
echo.
echo Setting DNS servers to Cloudflare (1.1.1.1 / 1.0.0.1)...
for /f "tokens=1*" %%A in ('netsh interface show interface ^| findstr /R /C:"Connected"') do (
    for /f "tokens=1,* delims=:" %%G in ('netsh interface ipv4 show config name^="%%B" ^| findstr "Configuration for"') do (
        set INTERFACE=%%B
        netsh interface ip set dns name="%%B" static 1.1.1.1 >nul 2>&1
        netsh interface ip add dns name="%%B" 1.0.0.1 index=2 >nul 2>&1
        echo DNS successfully set on adapter: %%B
    )
)
goto ASK
